...
if not API_KEY:
    ...
    logging.error("books Log - Erro: A variável de ambiente FIRECRAWL_API_KEY não foi definida.")
...
DUCKDB_FILE = "books_dados.duckdb"
...
def books_scrape_store_analyze():

    # Registra o início do processo
    logging.info("books Log - Iniciando o processo de scraping, armazenamento e análise...")

    # Registra a inicialização do aplicativo Firecrawl
    logging.info("books Log - Inicializando FirecrawlApp...")
    ...
    # Captura qualquer exceção que ocorra durante a inicialização
        ...
        logging.error(f"books Log - Falha ao inicializar FirecrawlApp: {e}")
    ...
    if EXTRACTOR_OPTIONS:
        ...
        logging.warning("books Log - Opções de Extractor definidas, mas não usadas nesta versão do script.")

    # Registra a URL que será raspada
    logging.info(f"books Log - Preparando para raspar URL: {URL_TO_SCRAPE}")

    # Registra os parâmetros que serão usados para o scraping, formatados como JSON para legibilidade
    logging.info(f"books Log - Usando parâmetros: {json.dumps(scrape_params, indent = 2)}")
    ...
    try:
        ...
        logging.info(f"books Log - Raspando URL: {URL_TO_SCRAPE}...")
        ...
        if scraped_data and 'html' in scraped_data and scraped_data['html']:
            ...
            logging.info("books Log - HTML obtido com sucesso.")

        # Caso contrário (se o HTML não for obtido).
        else:

            # Registra um erro indicando falha na obtenção do HTML
            logging.error("books Log - Falha ao obter HTML nos dados raspados.")
    ...
    # Captura qualquer exceção que ocorra durante o scraping
        ...
        logging.error(f"books Log - Ocorreu um erro durante o scraping: {e}", exc_info = False)
    ...
    if html_content:
        ...
        logging.info("books Log - Iniciando parsing do HTML e armazenamento no DuckDB...")
        ...
        try:
            ...
            logging.info(f"books Log - Encontrados {len(product_pods)} 'product_pods' para processar.")
            ...
            for pod in product_pods:
                ...
                if price_tag:
                    ...
                    # Captura um erro se a conversão para float falhar
                        ...
                        logging.warning(f"books Log - Não foi possível converter o preço '{price_text}' para número para o livro '{title}'.")
                ...
                # Caso contrário (se título ou preço estiverem faltando)
                    ...
                    logging.warning(f"books Log - Dados incompletos para um livro. Título: {title}, Preço: {price}")
            ...
            if not extracted_books:
                ...
                logging.warning("books Log - Nenhum dado de livro válido extraído do HTML.")
    
                # Retorna da função se não houver livros extraídos
                return

            # Registra quantos livros com título e preço válidos foram extraídos
            logging.info(f"books Log - Extraídos {len(extracted_books)} livros com título e preço válidos.")
            ...
            with duckdb.connect(database = DUCKDB_FILE, read_only = False) as con:
                ...
                logging.info(f"books Log - Conectado ao banco de dados DuckDB: {DUCKDB_FILE}")
                ...
                logging.info("books Log - Tabela 'books' verificada/criada.")
                ...
                logging.info("books Log - Dados antigos da tabela 'books' removidos.")
                ...
                logging.info(f"books Log - {len(extracted_books)} registros inseridos na tabela 'books'.")
        ...
        # Captura qualquer exceção que ocorra durante o parsing ou interação com o DuckDB
            ...
            logging.error(f"books Log - Erro durante o parsing ou interação com DuckDB: {db_err}", exc_info = True)

            # Retorna da função se ocorrer um erro
            return

        # Registra o início da seção de análise de dados no DuckDB
        logging.info("books Log - Iniciando a análise de preço médio via DuckDB.")
        ...
        try:
            ...
            with duckdb.connect(database = DUCKDB_FILE, read_only = True) as con:
                ...
                if result and result[0] is not None:
                    ...
                    logging.info(f"books Log - Consulta SQL executada no DuckDB.")

                    # Registra o número de livros usados para o cálculo da média
                    logging.info(f"books Log - Número de livros na tabela para cálculo: {book_count}")

                    # Registra o preço médio calculado, formatado com duas casas decimais
                    logging.info(f"books Log - PREÇO MÉDIO (calculado pelo DuckDB): £{average_price:.2f}")

                # Caso contrário (se não foi possível calcular a média)
                else:

                    # Registra um aviso indicando que a média não pôde ser calculada
                    logging.warning("books Log - Não foi possível calcular a média no DuckDB (tabela vazia ou erro na consulta?).")
        ...
        # Captura qualquer exceção que ocorra durante a análise no DuckDB
            ...
            logging.error(f"books Log - Erro durante a análise no DuckDB: {analysis_err}", exc_info = True)

        # Registra o fim da seção de análise
        logging.info("books Log - Análise concluída.")
