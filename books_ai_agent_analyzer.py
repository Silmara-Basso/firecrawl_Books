# Análise de Dados com SQL AI Agent e DuckDB
...
if not os.getenv("OPENAI_API_KEY"):
    logging.error("books Log - Erro Crítico: Variável de ambiente OPENAI_API_KEY não encontrada!")
    logging.error("books Log - Certifique-se de que sua chave está no arquivo .env ou definida no sistema.")
    exit(1)  
...
def books_analise_sql_agente_ia(question: str):

    # Registra o início da análise com o agente
    logging.info(f"books Log - Iniciando Análise com SQL AI Agent Para a Pergunta: '{question}'")

    # Verifica se o arquivo do banco de dados existe
    if not os.path.exists(DUCKDB_FILE):
        logging.error(f"books Log - Erro: Arquivo de banco de dados '{DUCKDB_FILE}' não encontrado.")
        logging.error("books Log - Execute o script de scraping primeiro para criar e popular o banco de dados.")
        return  
    ...
    try:
        ...
        logging.info(f"books Log - Conectando ao DB via LangChain com URI: {db_uri}")
        ...
        logging.info(f"books Log - LLM inicializado: {llm.model_name}")
        ...
        logging.info("books Log - Agente SQL criado com sucesso.")

        # Registra o início da execução do agente com a pergunta fornecida
        logging.info(f"books Log - Invocando agente com a pergunta: '{question}'")

        # Invoca o agente com a pergunta e obtém a resposta
        response = agent_executor.invoke({"input": question})

        # Registra o término da execução do agente
        logging.info("books Log - Agente concluiu a execução.")

        # Registra a resposta final do agente
        logging.info("books Log - Resposta Final do Agente:")
        ...
        with open("books_perguntas_respostas.txt", "a", encoding = "utf-8") as arquivo:
            arquivo.write(f"{'='*40}\n")
        ...
        logging.error(f"books Log - Erro de importação. Verifique as instalações: {ie}")
        logging.error("books Log - Certifique-se de ter instalado as dependências.")

    except Exception as e:
        # Trata outros erros inesperados
        logging.error(f"books Log - Ocorreu um erro inesperado durante a análise com o agente: {e}", exc_info = True)
...
if __name__ == "__main__":
    ...
    books_analise_sql_agente_ia("Qual o preço médio dos livros?")
    books_analise_sql_agente_ia("Liste os 3 livros mais caros, incluindo seus preços.")
    books_analise_sql_agente_ia("Qual o livro mais barato?")
    books_analise_sql_agente_ia("Quantos livros custam mais de 50 libras?")
    books_analise_sql_agente_ia("Mostre o título e o preço do livro chamado 'Set Me Free'.")

    # Registra o término do processo de análise
    logging.info("Processo de análise finalizado. Obrigado books.")
